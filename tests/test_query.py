...
class TestQueryMessages:
    ...
    async def test_pagination_bounds_groups(self, session):
        ...
        await _seed_alerts(
            session,
            [{"severity": "warn", "category": "flood", "message": f"task-{i}"} for i in range(25)],
        )
...
class TestQueryOccurrences:
    ...
    async def test_pagination(self, session):
        ...
        await _seed_alerts(
            session,
            [{"severity": "warn", "category": "cat", "message": f"m{i}"} for i in range(10)],
        )
    ...
    async def test_limits_tracebacks_to_3(self, session):
        ...
        for i in range(5):
            await fire_alert(
                ...
                category="cat",
                message="m",
                context={"_exc_type": f"Err{i}", "_exc_message": f"m{i}", "_traceback": f"tb {i}"},
            )
        await session.commit()
...
class TestDeleteBatch:
    ...
    async def test_deletes_only_exact_category_and_message(self, session):
        ...
        count = await delete_batch(session, category="cat", message="boom", before_id=rows[-1].id)
        await session.commit()
        assert count == 2
